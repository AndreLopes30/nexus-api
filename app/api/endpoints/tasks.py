...
from app.schemas.task import atualizarTarefa, criarTarefa, lerTarefa
...
def get_owner(db: Session, current_user: str) -> User:
    owner = (
        db.query(User).filter((User.email == current_user) | (User.nome == current_user)).first()
    )
    if not owner:
        raise HTTPException(
...
def create_task(
    ...
    owner = get_owner(db, current_user)
    tarefa_db = Task(
        title=tarefa.title, description=tarefa.description, owner_id=owner.id, done=False
    )
    db.add(tarefa_db)
    db.commit()
...
def delete_task(
    task_id: int, current_user: str = Depends(get_current_user), db: Session = Depends(get_db)
):
